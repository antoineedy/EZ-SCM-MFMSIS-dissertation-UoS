...
@BACKBONES.register_module()
class DPTCLIPTextEncoder(nn.Module):
    def __init__(self, context_length=77,
                 vocab_size=49408,
                 transformer_width=512,
                 transformer_heads=8,
        ...
        # Positional and prompt embeddings
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))
        self.prompt_embedding = nn.Parameter(torch.empty(self.num_prompt_tokens, transformer_width))
    ...
    def build_attention_mask(self):
        # lazily create causal attention mask, with full attention between the vision tokens
        # pytorch uses additive attention mask; fill with -inf
        mask = torch.empty(self.context_length + self.num_prompt_tokens, self.context_length + self.num_prompt_tokens)
        mask.fill_(float("-inf"))
        mask.triu_(1)  # zero out the lower diagonal
        return mask

    def forward(self, text):
        # Embedding and prompt insertion
        x = self.token_embedding(text)
        x = x + self.positional_embedding[:x.size(1), :]  # Positional embeddings

        # Insert prompt tokens at the start
        prompt_tokens = self.prompt_embedding.unsqueeze(0).expand(x.size(0), -1, -1)
        ...
        # Select the output corresponding to the last token of the original text (excluding prompt tokens)
        x = x[:, self.num_prompt_tokens + torch.arange(text.shape[1]), text.argmax(dim=-1)] @ self.text_projection
