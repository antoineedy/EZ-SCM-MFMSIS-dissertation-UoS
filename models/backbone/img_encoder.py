...
class CLIPVisionTransformer(nn.Module):
    def __init__(
        ...
        layers=12,
        heads=12,
        output_dim=512,  # ? output dim is not 768?
        drop_path_rate=0.0,
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], -1)  # antoine: reshape the tensor
        x = x.permute(0, 2, 1)
        x = torch.cat(
            [
...
class VPTCLIPVisionTransformer(nn.Module):
    def __init__(
        ...
        output_dim=512,  # antoine: output_dim is the dimension of the output
        drop_path_rate=0.0,
        out_indices=[
            3,
            5,
