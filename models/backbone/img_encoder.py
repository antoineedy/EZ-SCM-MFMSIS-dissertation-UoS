...
@BACKBONES.register_module()
class InnerVPTCLIPVisionTransformer(nn.Module):
    def __init__(
        self,
        input_resolution=224,  # antoine: input_resolution is the size of the input image
        patch_size=32,  # antoine: patch_size is the size of the patch
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.total_d_layer == 0:  # shallow
            ...
            x, features, cls_tokens = self.forward_deep_prompt(x, features, H, W)
        elif self.total_d_layer < 0:
            x, features, cls_tokens = self.forward_reverse_deep_prompt(
                x, features, H, W
            )
        else:
            AttributeError("Input correct total_d_layer")
    ...
    def forward_deep_prompt(self, embedding_output, features, H, W, out_last=False):
        ...
        for i in range(self.num_layers):
            ...
            if i in [5, 7, 11]:
                cls_tokens[i + 1] = hidden_states[
                    0, :, :
                ].clone()  # CLS token is the first token (index 0)
        ...
        if out_last:
            return before_last_feats, cls_tokens
        else:
            return encoded, features, cls_tokens
