...
class InnerVPTCLIPVisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        cls_tokens = []  # New list to store cls_tokens
        ...
        if self.total_d_layer == 0:  # shallow
            for i, blk in enumerate(self.transformer.resblocks):
                ...
                # Store cls_token for layers 6, 8, and 12
                if i in [5, 7, 11]:  # Layers are zero-indexed
                    cls_token = x.clone()  # Store the cls_token (global embedding)
                    cls_token = cls_token.permute(1, 0, 2)
                    cls_token = self.ln_post(x)
                    cls_token = x @ self.proj
                    cls_token = x[:, 0]
