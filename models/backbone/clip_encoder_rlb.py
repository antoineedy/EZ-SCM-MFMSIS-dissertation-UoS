...
class CLIPVisionTransformerWithRLB(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.region_level_bridge_hw = int(math.sqrt(self.region_level_bridge_size))
        self.region_level_bridge = jt.zeros(self.region_level_bridge_size, prompt_dim)

        visual_mask = self.gen_attention_mask(
            stride=self.spatial_size // self.region_level_bridge_hw
        )
    ...
    def gen_attention_mask(self, stride):
        ...
        att_size = (
            1 + self.num_tokens + self.spatial_size**2 + self.region_level_bridge_size
        )
        rlb_index = -self.region_level_bridge_size
        visual_mask = jt.zeros((att_size, att_size), dtype=jt.float32).stop_grad()
        ...
        for i in range(self.region_level_bridge_hw):
            for j in range(self.region_level_bridge_hw):
                tmp_mask = jt.zeros((self.spatial_size, self.spatial_size)).stop_grad()
                tmp_mask[
                    i * stride : (i + 1) * stride, j * stride : (j + 1) * stride
                ] = 1

                idx = tmp_mask.flatten().nonzero() + self.num_tokens + 1
                visual_mask[rlb_index + i * self.region_level_bridge_hw + j, idx] = 0

                visual_mask[idx, rlb_index + i * self.region_level_bridge_hw + j] = 0
    ...
    def _init_prompt(self, patch, num_tokens, prompt_dim, total_d_layer):
        ...
        if total_d_layer >= 0:
            ...
            if total_d_layer > 0:  # noqa
                self.deep_prompt_embeddings = jt.zeros(
        ...
        if isinstance(pretrained, str):
            ...
            if "positional_embedding" in state_dict.keys():
                if (
                    ...
                    cls_pos = jt.Var(state_dict["positional_embedding"][0:1, :])

                    spatial_pos = nn.interpolate(
                        jt.Var(state_dict["positional_embedding"])[1:,]
                        .reshape(1, 14, 14, 768)
                        .permute(0, 3, 1, 2),
                    ...
                    positional_embedding = jt.concat([cls_pos, spatial_pos], dim=0)
            ...
            region_level_bridge_init = jt.Var(state_dict["class_embedding"]).repeat(
    ...
    def execute(self, x: jt.Var):
        ...
        x = jt.concat(
            [
                ...
                + jt.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype),
                x,
            ],
            dim=1,
        )
        ...
        x = jt.concat(
            [x, self.region_level_bridge.unsqueeze(0).expand(B, -1, -1)], dim=1
        )
        x = self.ln_pre(x)
        ...
        if self.get_embeddings:
            ...
            visual_embedding = (
                x[:, 1 + self.num_tokens : -self.region_level_bridge_size]
                .reshape(B, H, W, -1)
                .permute(0, 3, 1, 2)
            )
            region_level_bridge = (
                x[:, -self.region_level_bridge_size :]
                .reshape(
                    B, self.region_level_bridge_hw, self.region_level_bridge_hw, -1
                )
                .permute(0, 3, 1, 2)
            )
    ...
    def execute_deep_prompt(self, embedding_output, features, H, W, out_last=False):
                ...
                if i in self.out_indices:
                    xp = (
                        hidden_states.permute(1, 0, 2)[
                            :, (1 + self.num_tokens) : -self.region_level_bridge_size, :
                        ]
                        .permute(0, 2, 1)
                        .reshape(B, -1, H, W)
                    )
        ...
        encoded = jt.concat(
            (
                self.prompt_norm(hidden_states[: -self.region_level_bridge_size, :, :]),
