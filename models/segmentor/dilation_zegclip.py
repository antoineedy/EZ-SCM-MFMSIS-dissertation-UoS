...
class DilationZegCLIP(EncoderDecoder):
    ...
    def __init__(
        ...
        **args,
        ...
        super(DilationZegCLIP, self).__init__(**args)
        ...
        self.conv_mult_dilu_1 = nn.Conv2d(512, 512, 3, stride = 1, padding=1, dilation = 1)

        # torch.Size([1, 512, 32, 32]) -> torch.Size([1, 512, 32, 32])
        self.conv_mult_dilu_2 = nn.Conv2d(512, 512, 3, stride = 1, padding=2, dilation = 2)

        # torch.Size([1, 512, 32, 32]) -> torch.Size([1, 512, 32, 32])
        self.conv_mult_dilu_3 = nn.Conv2d(512, 512, 3, stride = 1, padding=3, dilation = 3)
    ...
    def mutli_scale_inner(self, visual_feat):
        ...
        # most precise
        layer6 = self.conv_mult_dilu_1(layer6)
        
        layer8 = self.conv_mult_dilu_2(layer8)

        # least precise
        layer12 = self.conv_mult_dilu_3(layer12)

        weights = [1, 1, 1]/3
        w6, w8, w12 = weights

        layer = w6 * layer6 + w8 * layer8 + w12 * layer12
